...
for p in ps:
    ...
    for q in qs:
        ...
        plt.plot(x_axis, x_axis*(x_axis+1)*column/(2*np.pi), color=colors[i].rgb, label="q={0}" .format(q))
        i += 1
    ...
    plt.savefig('figures/power2D/p={0}/power_q.png' .format(p), dpi=200, bbox_inches='tight')
    plt.clf()
    print(p)
# plt.show()
