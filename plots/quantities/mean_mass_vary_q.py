...
for a in a_s:
    for p in ps:
        ...
        plt.xlabel('M')
        plt.ylabel('$Mn(M)$')
        plt.title("Mean mass function varying $q$, $p=${0}, $a=${1}" .format(*[p, a]))
        plt.legend()
        os.system('mkdir -p figures/quantities/p={0}/a={1}' .format(*[p, a]))
        plt.savefig('figures/quantities/p={0}/a={1}/mean_mass_function_q.png' .format(*[p, a]), dpi=200, bbox_inches='tight')
        plt.clf()
    print("a=" + str(a))
