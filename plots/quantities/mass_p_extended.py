...
for index_a in range(1, len(a_s)+1):
    ...
    for q in qs:
        ...
        os.system('mkdir -p figures/mass/a={1}/q={0}' .format(*[q, a]))
        ...
        i = 0
        for p in ps:
            data = open('data/q={0}p={1}/mass_{2}.dat' .format(*[q, p, index_a]))
            ...
            i += 1
        plt.title("Mass function varying $p$, $q=${0}, $a=${1}" .format(*[q, a]))
        plt.legend()
        plt.savefig('figures/mass/a={1}/q={0}/mass_p.png' .format(*[q, a]), bbox_inches='tight')
        plt.clf()
    print("a=" + str(a))
# plt.show()
