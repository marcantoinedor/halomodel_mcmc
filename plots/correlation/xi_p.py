...
for q in qs:
    for p in ps:
        if clean:
            os.system('mv -f data/q={0}p={1}/xi1.dat data/q={0}p={1}/xi1_old.dat'.format(*[q, p]))
            os.system('mv -f data/q={0}p={1}/xi3.dat data/q={0}p={1}/xi3_old.dat'.format(*[q, p]))

        if not os.path.isfile('data/q={0}p={1}/xi1.dat'.format(*[q, p])):
            print("Creating data {0}" .format(p))
            os.system('./bin/halo_model_plotxi {0} {1} {2}' .format(*[q, p, l_max]))

    os.system('mkdir -p figures/corrFunction/q={0}' .format(q))
    ...
    i = 0
    x_axis = []
    data = open('data/q={0}p={1}/xi1.dat' .format(*[qs[0], ps[0]]))
    ...
    plt.errorbar(thetasCFHT, xipCFHT, sigpCFHT, fmt='o')
    plt.legend()
    # os.system('mv figures/corrFunction/q={0}/xip.png figures/corrFunction/q={0}/xip_old.png' .format(q))
    plt.savefig('figures/corrFunction/q={0}/xip_CFHT.png' .format(q), bbox_inches='tight')
    plt.clf()

    plt.figure(3).set_size_inches((8, 8), forward=False)
    plt.title("Correlation function varying $p$, $q=${0}" .format(q))
    plt.errorbar(thetasCFHT, ximCFHT, sigmCFHT, fmt='o')
    plt.legend()
    # os.system('mv figures/corrFunction/q={0}/xim.png figures/corrFunction/q={0}/xip_old.png' .format(q))
    plt.savefig('figures/corrFunction/q={0}/xim_CFHT.png' .format(q), bbox_inches='tight')
    plt.clf()
    print("q={0}" .format(q))
# plt.show()
