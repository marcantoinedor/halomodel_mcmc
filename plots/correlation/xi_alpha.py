...
create.xi_CFHT_alpha(alphas, l_max, clean=clean)
...
for alpha in alphas:
    ...
    column1 = dat.get_xip_alpha(alpha)
    column3 = dat.get_xim_alpha(alpha)

    plt.figure(1)
    plt.plot(x_axis, column1, color=colors[i].rgb, label="$\\alpha={0}$" .format(alpha))
    plt.figure(3)
    plt.plot(x_axis, column3, color=colors[i].rgb, label="$\\alpha={0}$" .format(alpha))
    i += 1
...
if CFHT_data:
    ...
    plt.savefig('figures/correlation/xip_CFHT_alpha.png', dpi=200, bbox_inches='tight')
else:
    plt.savefig('figures/correlation/xip_alpha.png', dpi=200, bbox_inches='tight')
...
if CFHT_data:
    ...
    plt.savefig('figures/correlation/xim_CFHT_alpha.png', dpi=200, bbox_inches='tight')
else:
    plt.savefig('figures/correlation/xim_alpha.png', dpi=200, bbox_inches='tight')
plt.show()
