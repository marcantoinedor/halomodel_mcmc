...
plt.errorbar(x_axis, xipCFHT, np.sqrt(sigpCFHT), fmt='.k', elinewidth=0.5, capsize=3)
plt.legend()
# os.system('mv figures/corrFunction/q={0}/xip.png figures/corrFunction/q={0}/xip_old.png' .format(q))
plt.savefig('figures/icosmo/xip', dpi=300, bbox_inches='tight')

plt.figure(3).set_size_inches((8, 8), forward=False)
plt.title("Correlation function varying cosmology")
plt.errorbar(x_axis, ximCFHT, np.sqrt(sigmCFHT), fmt='.k', elinewidth=0.5, capsize=3)
plt.legend()
# os.system('mv figures/corrFunction/q={0}/xim.png figures/corrFunction/q={0}/xip_old.png' .format(q))
plt.savefig('figures/icosmo/xim', dpi=300, bbox_inches='tight')
plt.show()
