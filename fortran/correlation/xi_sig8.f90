PROGRAM halo_model
   ...
   IMPLICIT NONE
   REAL ::power_f, mass, l_length_real, kmin, kmax, amin, amax, lmin, lmax, thmin, thmax, l_max_real, sig8
   ...
   CHARACTER(len=256) :: fbase, fbase2, fext1, fext3, output1, output3, l_max_str, input, th_str, sig8_str, ext
   TYPE(halomod) :: hmod
   TYPE(cosmology) :: cosm
   LOGICAL :: verbose2
   ...
   CALL get_command_argument(1, sig8_str)
   read (sig8_str, '(f10.0)') sig8
   cosm%sig8 = sig8
   ...
   fbase = 'data/sig8='
   fbase = TRIM(fbase)//TRIM(sig8_str)
   fext1 = '/xi1.dat'
