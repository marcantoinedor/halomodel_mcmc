...
mmin = 1e12
mmax = 1e15
x = np.exp(np.linspace(np.log(mmin), np.log(mmax), N))

# writting mass axis to a file for fortran code

masses = open("data/mass.dat", "w")
for mass in x:
    masses.write(str(mass)+"\n")
masses.close()

y = realMf.multiplicity_functionv(N, q_st, p_st, 1.0)
...
def lnlike(theta, mass, y, yerr, a):
    q, p = theta
    model = realMf.multiplicity_functionv(mass, q, p, a)
