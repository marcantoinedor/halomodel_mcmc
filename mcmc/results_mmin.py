...
data = open("mcmc/results/{1}/ihm={2}/mmin{0}.txt" .format(*[icosmo, usedData, ihm]), "r")
line = data.readlines()
data.close()

mmin_ml = float(line[0].split('=')[1])
...
chain = np.load('mcmc/results/{1}/ihm={2}/mmin_chain{0}.npy' .format(*[icosmo, usedData, ihm]))
...
if usedData == 'CFHT':
    model = correlation.xi_mminCFHT(mmin_ml, icosmo, ihm, True)
    theory = correlation.xi_mminCFHT(mmin_st, icosmo, ihm, True)
...
plt.plot(x, theory[:N], '--', label="Fiducial")
plt.legend()
...
plt.savefig('mcmc/figures/{1}/ihm={2}/xip_mmin{0}.png' .format(*[icosmo, usedData, ihm]), bbox_inches='tight', dpi=200)
...
plt.xlabel('$\\theta (arcmin)$')
plt.ylabel('$\\xi_-$')
plt.savefig('mcmc/figures/{1}/ihm={2}/xim_mmin{0}.png' .format(*[icosmo, usedData, ihm]), bbox_inches='tight', dpi=200)
...
plt.figure(3)
plt.savefig("mcmc/figures/{1}/ihm={2}/mcmc_walkers_mmin{0}.png" .format(*[icosmo, usedData, ihm]), dpi=200)

fig = corner.corner(samples, labels=["$log(M_{min}/M_{\\odot})$"], truths=[mmin_ml])
fig.savefig("mcmc/figures/{1}/ihm={2}/mcmc_contours_mmin{0}.png" .format(*[icosmo, usedData, ihm]), dpi=200)
...
if percentiles:
    ...
    # Compute contours
    upper_percentile1 = correlation.xi_mminCFHT(q_mcmc1[0]+q_mcmc1[1], p_mcmc1[0]+p_mcmc1[1], icosmo, ihm, True)
    lower_percentile1 = correlation.xi_mminCFHT(q_mcmc1[0]-q_mcmc1[2], p_mcmc1[0]-p_mcmc1[2], icosmo, ihm, True)

    upper_percentile2 = correlation.xi_mminCFHT(q_mcmc2[0]+q_mcmc2[1], p_mcmc2[0]+p_mcmc2[1], icosmo, ihm, True)
    lower_percentile2 = correlation.xi_mminCFHT(q_mcmc2[0]-q_mcmc2[2], p_mcmc2[0]-p_mcmc2[2], icosmo, ihm, True)
...
for mmin in samples[np.random.randint(len(samples), size=nbr)]:
    values = correlation.xi_mminCFHT(mmin[0], icosmo, ihm, True)
    plt.figure(5)
...
plt.figure(5)
plt.savefig('mcmc/figures/{1}/ihm={2}/xip_mmin_var{0}.png' .format(*[icosmo, usedData, ihm]), bbox_inches='tight', dpi=200)
plt.figure(6)
plt.savefig('mcmc/figures/{1}/ihm={2}/xim_mmin_var{0}.png' .format(*[icosmo, usedData, ihm]), bbox_inches='tight', dpi=200)
